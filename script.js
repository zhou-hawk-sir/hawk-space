...
async function handleRegister(email, password) {
    if (!cloudbaseApp) {
        // 云服务不可用，直接使用本地模式
        await handleLocalRegister(email, password);
        return;
    }
...
// 本地注册逻辑（备用方案）
async function handleLocalRegister(email, password) {
    const registeredUsers = JSON.parse(localStorage.getItem('registeredUsers') || '{}');
    ...
    if (registeredUsers[email]) {
        ...
        registeredUsers[email] = btoa(password);
