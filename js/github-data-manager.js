...
class GitHubDataManager {
    constructor() {
        ...
        this.token = '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'; // 你的token - 后期要移除！
    ...
    async getFileContent(filePath) {
        try {
            ...
            if (response.status === 404) {
                ...
                return {
                    content: btoa(JSON.stringify(emptyData)),
                    sha: null
                };
            }
    ...
    async verifyUser(email, password) {
        try {
            ...
            const usersContent = JSON.parse(atob(usersFile.content));
    ...
    async recordLogin(userId, loginInfo) {
        try {
            ...
            const sessionsContent = JSON.parse(atob(sessionsFile.content)) || { sessions: [] };
    ...
    async getAllUsers() {
        try {
            const usersFile = await this.getFileContent('user/users.json');
            return JSON.parse(atob(usersFile.content)).users;
        } catch (error) {
    ...
    async getLoginHistory() {
        try {
            const sessionsFile = await this.getFileContent('sessions/active-sessions.json');
            return JSON.parse(atob(sessionsFile.content)).sessions || [];
        } catch (error) {
