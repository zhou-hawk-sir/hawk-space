...
class GitHubDataManager {
    ...
    async getLocationInfo(ip = null) {
        try {
            // 如果是本地IP，直接返回本地环境
            if (!ip || ip === '127.0.0.1' || ip === 'localhost' || ip === '未知' || ip.startsWith('192.168.') || ip.startsWith('10.') || ip.startsWith('172.')) {
                return {
                    country: '中国',
                    region: '本地网络',
                    city: '本地主机',
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.status === 'success') {
                        return {
                            ...
                            city: data.city || '未知',
                            isp: data.isp || '未知',
                            fullLocation: this.formatDetailedLocation(data),
                            latitude: data.lat,
                            longitude: data.lon,
                ...
                console.log('ip-api.com 请求失败，尝试备用API');
            }

            // 方法2: 使用 ipapi.co API (提供详细位置信息)
            try {
                ...
                if (response.ok) {
                    ...
                    return {
                        ...
                        region: data.region || data.region_code || '未知',
                        city: data.city || '未知',
                        isp: data.org || '未知',
                        fullLocation: this.formatDetailedLocation(data),
    ...
    async recordLoginWithUserLocation(userId, loginInfo) {
        try {
            ...
            try {
                ...
                console.log('用户位置获取失败，使用IP位置:', locationError.message);
                // 使用IP定位作为备选
                const ipAddress = await this.getIPAddress();
                userLocation = await this.getLocationInfo(ipAddress);
            }
