...
// 在 github-data-manager-secure.js 末尾添加这些方法
...
GitHubDataManager.prototype.addProduct = async function(productData) {
    try {
        const productsFile = await this.getFileContent('products/products.json');
        const productsContent = this.safeAtob(productsFile.content) || { products: [] };
        ...
        const newProduct = {
            ...
            location: productData.location || '未知位置',
...
GitHubDataManager.prototype.getOnlineUsers = async function() {
    try {
        const onlineFile = await this.getFileContent('sessions/online-users.json');
        const onlineContent = this.safeAtob(onlineFile.content);
        return onlineContent.users || [];
    } catch (error) {
...
GitHubDataManager.prototype.updateOnlineStatus = async function(userId, userEmail, status = 'online') {
    try {
        const onlineFile = await this.getFileContent('sessions/online-users.json');
        const onlineContent = this.safeAtob(onlineFile.content) || { users: [] };

        const existingIndex = onlineContent.users.findIndex(u => u.userId === userId);

        if (existingIndex !== -1) {
            onlineContent.users[existingIndex].lastSeen = new Date().toISOString();
            onlineContent.users[existingIndex].status = status;
        } else {
            onlineContent.users.push({
                userId: userId,
                userEmail: userEmail,
                status: status,
        ...
        onlineContent.users = onlineContent.users.filter(u => u.lastSeen > oneHourAgo);

        await this.updateFile('sessions/online-users.json', onlineContent, onlineFile.sha);
    } catch (error) {
        console.error('更新在线状态失败:', error);
    }
};
// 聊天数据管理方法
GitHubDataManager.prototype.getChatMessages = async function(roomId = 'main') {
    try {
        const chatFile = await this.getFileContent(`chat/rooms/${roomId}.json`);
        const chatContent = this.safeAtob(chatFile.content);
...
GitHubDataManager.prototype.sendMessage = async function(roomId, messageData) {
    try {
        const chatFile = await this.getFileContent(`chat/rooms/${roomId}.json`);
        const chatContent = this.safeAtob(chatFile.content) || { messages: [] };
        ...
        // 只保留最近500条消息
        if (chatContent.messages.length > 500) {
            chatContent.messages = chatContent.messages.slice(-500);
        }

        await this.updateFile(`chat/rooms/${roomId}.json`, chatContent, chatFile.sha);
        return newMessage;
    } catch (error) {
...
GitHubDataManager.prototype.getChatRooms = async function() {
    try {
        const roomsFile = await this.getFileContent('chat/rooms.json');
        const roomsContent = this.safeAtob(roomsFile.content);
