[file name]: js/admin-dashboard.js
[file content begin]
// 管理员数据看板逻辑
class AdminDashboard {
    ...
    renderLoginHistory() {
        ...
        const recentLogins = this.loginHistory.slice(0, 50); // 只显示最近50条
        ...
        tbody.innerHTML = recentLogins.map(login => `
            <tr>
                <td>${this.getShortEmail(login.userId)}</td>
                <td>${this.formatDateTime(login.loginTime)}</td>
                <td>${login.ipAddress || '未知'}</td>
                <td>${login.location || '自动获取中...'}</td>
                <td>${this.getBrowserInfo(login.userAgent)}</td>
                <td>${login.deviceType || '未知'}</td>
                <td>
